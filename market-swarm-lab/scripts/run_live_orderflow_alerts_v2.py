#!/usr/bin/env python3
"""
Live Orderflow Alert Engine — v2.1 (Dry-Run Mode)

FIXES:
- Permissive ES symbol filtering: matches any symbol containing "ES" or "ESH"
- Event sequence handling: seq starts at 1 (monotonic)

Usage:
    python scripts/run_live_orderflow_alerts_v2.py \
...
LIVE_STATE_DIR = ROOT / "state" / "orderflow" / "live"
            ...
            with open(file_path, "r", errors="replace") as f:
                ...
                for line in f:
                    ...
                    try:
                        ev = json.loads(line)
                        seq = ev.get("seq", 0)
                        # FIX: seq is monotonic starting from 1, not 0
                        # Only skip if seq <= last_seq (handles both old and new events)
                        if seq > cp.last_seq or cp.last_seq == 0:
                            events.append(ev)
                            cp.last_seq = seq
...
class ConfidenceScorer:
    ...
    def compute(self, sweep: SweepEvent, footprint: Dict, spy_data: Optional[Dict]) -> int:
        ...
        # SPY confirmation (simplified for cached mode)
        if spy_data:
...
class LiveOrderflowAlertEngine:
    def __init__(self,
                 ...
                 spy_source: str = "cached",
                 notify_mode: str = "none",
                 confidence_threshold: int = DEFAULT_CONFIDENCE_THRESHOLD,
                 cooldown_minutes: int = DEFAULT_COOLDOWN_MINUTES,
                 dry_run: bool = True,
                 tail_from_eof: bool = True):
        ...
        self.tail_from_eof = tail_from_eof
    ...
    def _get_spy_data(self) -> Optional[Dict]:
        """Get SPY data - cached mode returns static data."""
    ...
    def _initialize_checkpoints(self):
        """Set checkpoints to EOF to skip historical data."""
        if not self.tail_from_eof:
            return
    ...
    def process_cycle(self) -> List[AlertSignal]:
        ...
        # FIX: Permissive ES symbol filtering
        # Match any symbol containing "ES" or starts with "ESH" (e.g. "ESH5.CME@BMD")
        es_events = [
            e for e in new_events
            if "ES" in e.get("symbol", "") or e.get("symbol", "").startswith("ESH")
        ]
        if not es_events:
    ...
    def run(self, interval_seconds: float = 5.0):
        ...
        # Initialize checkpoints (tail from EOF)
        self._initialize_checkpoints()

        _log.info("=" * 60)
        _log.info("Live Orderflow Alert Engine v2.1")
        ...
        _log.info("Tail from EOF: %s", "YES" if self.tail_from_eof else "NO")
        _log.info("=" * 60)

        # Write startup status
        self._write_status([
            f"Engine v2.1 started",
            f"SPY source: {self.spy_source}",
            f"Dry run: {self.dry_run}",
            f"Tail from EOF: {self.tail_from_eof}",
        ], force=True)
        ...
        while self.running:
            ...
            for signal in signals:
                if self.alert_mgr.can_alert(signal):
                    ...
                    self._write_status([
                        f"SIGNAL: {signal.direction} {signal.underlying} "
...
def main():
    parser = argparse.ArgumentParser(
        description="Live Orderflow Alert Engine v2.1 — alerts only, no trading"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--spy-source", default="cached", choices=["cached"],
        help="Source for SPY data (cached only for v2)"
    )
    parser.add_argument(
        "--notify", default="none", choices=["none"],
        help="Notification method (none for v2)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Alert-only mode (default: true)"
    )
    parser.add_argument(
        "--tail-from-eof", action="store_true", default=True,
        help="Start from current EOF, skip historical data"
    )
    ...
    engine = LiveOrderflowAlertEngine(
        ...
        tail_from_eof=args.tail_from_eof,
    )
