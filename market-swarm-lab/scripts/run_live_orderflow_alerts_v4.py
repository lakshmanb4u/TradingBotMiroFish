#!/usr/bin/env python3
"""Live Orderflow Alert Engine — v4.0 (Dry-Run Mode + Diagnostics)

No trading logic changes. Adds diagnostics tracking for every rejected setup.
"""
...
def init_diagnostics() -> None:
    """Initialize diagnostics CSV if not exists."""
...
def append_rejected(row: Dict[str, Any]) -> None:
    fieldnames = ["ts_rejected", "symbol", "setup_type", "confidence", "rejection_reason",
                  "missing_confirmations", "es_price", "nq_price", "spy_state", "suggestion"]
    try:
        ...
        _log.error("Failed to append rejected setup: %s", e)

def write_diagnostics_md(windows: List[DiagnosticsWindow]) -> None:
    lines = ["# Signal Diagnostics Report\n", f"Updated: {et_now().strftime('%Y-%m-%d %H:%M:%S ET')}\n", f"Windows tracked: {len(windows)}\n\n"]
    if not windows:
        lines.append("No data yet.\n")
    else:
        # Summary table
        lines.append("## Rejection Summary (Last 10 Windows)\n\n")
        lines.append("| Window | Events | ES | NQ | Sweeps | Reclaims | Movement | NoReclaim | Confidence | Cooldown | SPY | Stale | MaxConf | ES Price | NQ Price |\n")
        lines.append("|--------|--------|----|----|--------|----------|----------|-----------|------------|----------|-----|-------|---------|----------|----------|\n")
        for i, w in enumerate(windows[-10:], 1):
            lines.append(f"| {w.window_start} | {w.events_processed} | {w.es_events} | {w.nq_events} | {w.candidate_sweeps} | {w.reclaim_candidates} | {w.rejected_not_enough_movement} | {w.rejected_no_reclaim} | {w.rejected_confidence} | {w.rejected_cooldown} | {w.rejected_spy_failed} | {w.rejected_stale} | {w.highest_confidence_seen} | {w.latest_es_price} | {w.latest_nq_price} |\n")
        lines.append("\n## Root Cause Analysis\n\n")
        w = windows[-1]
        total_rejected = (w.rejected_not_enough_movement + w.rejected_no_reclaim +
                          w.rejected_confidence + w.rejected_cooldown +
                          w.rejected_spy_failed + w.rejected_stale)
        lines.append(f"- Total rejections last window: {total_rejected}\n")
        if total_rejected == 0 and w.candidate_sweeps == 0:
            lines.append("- **Root cause: No sweeps occurred in window**\n")
            lines.append("  → Price action is ranging without liquidity sweeps\n")
        elif w.rejected_not_enough_movement > 0:
            lines.append("- **Root cause: Price moves too small for sweep detection**\n")
            lines.append("  → Sweeps need 1-10 ticks ($0.25-$2.50 ES / $0.50-$5.00 NQ)\n")
        elif w.rejected_no_reclaim > 0:
            lines.append("- **Root cause: Sweeps without reclaim**\n")
            lines.append("  → Price swept level but never returned to it\n")
        elif w.rejected_confidence > 0:
            lines.append(f"- **Root cause: Confidence too low** (highest: {w.highest_confidence_seen}/100)\n")
            lines.append("  → Missing confirmations:\n")
            lines.append("    1. Delta exhaustion (ratio >2:1 against sweep)\n")
            lines.append("    2. Deeper sweep distance (need 4+ ticks for +15 pts)\n")
            lines.append("    3. SPY trend alignment (cached data may be stale)\n")
            lines.append("    4. Absorption at level (not yet computed)\n")
        elif w.rejected_cooldown > 0:
            lines.append("- **Root cause: Cooldown blocking**\n")
            lines.append("  → High-confidence signal exists but within 10-min cooldown\n")
        else:
            lines.append("- No rejections in last window\n")
    with open(DIAG_MD, "w") as f:
        f.writelines(lines)

def write_diagnostics_json(windows: List[DiagnosticsWindow]) -> None:
    data = {
        "updated": et_now().strftime('%Y-%m-%d %H:%M:%S ET'),
        "total_windows": len(windows),
        "latest_window": asdict(windows[-1]) if windows else {},
        "windows": [asdict(w) for w in windows[-20:]]
    }
...
# ─── Sweep Detector (unchanged logic) ────────────────────────────────────────
    ...
    def _check_sweep(self, ev: Dict) -> Optional[SweepEvent]:
        ...
        if p < support - self.min_sweep and p > support - self.max_sweep:
            ...
            if key not in self._pending:
                self._pending[key] = {"type": "bullish", "trigger_ts": ev.get("ts_event", ""),
        ...
        if p > resistance + self.min_sweep and p < resistance + self.max_sweep:
            ...
            if key not in self._pending:
                self._pending[key] = {"type": "bearish", "trigger_ts": ev.get("ts_event", ""),
                    "trigger_price": p, "level": resistance, "seq": seq}
        # Check reclaims
        for key, pending in list(self._pending.items()):
            if pending["type"] == "bullish" and p >= pending["level"]:
                sweep = SweepEvent(ts_event=pending["trigger_ts"], symbol=symbol,
                    direction="bullish_sweep", trigger_price=pending["trigger_price"], level=pending["level"],
                    sweep_distance=abs(pending["trigger_price"] - pending["level"]), reclaim_delay_ms=0.0,
                    liquidity_behavior=f"bid_size={self.latest_bid_size}", confidence=0.6, side=ev.get("side", ""),
                    size=ev.get("size", 0) or 0, seq=pending["seq"])
                del self._pending[key]
                return sweep
            elif pending["type"] == "bearish" and p <= pending["level"]:
                sweep = SweepEvent(ts_event=pending["trigger_ts"], symbol=symbol,
                    direction="bearish_sweep", trigger_price=pending["trigger_price"], level=pending["level"],
                    sweep_distance=abs(pending["trigger_price"] - pending["level"]), reclaim_delay_ms=0.0,
                    liquidity_behavior=f"ask_size={self.latest_ask_size}", confidence=0.6, side=ev.get("side", ""),
                    size=ev.get("size", 0) or 0, seq=pending["seq"])
                del self._pending[key]
                return sweep
        return None

# ─── Footprint Analyzer (unchanged logic) ────────────────────────────────────
    ...
    def process_trades(self, events: List[Dict]) -> Dict:
        ...
        while self.trades and parse_ts(self.trades[0].get("ts_event", "")) < cutoff:
            ...
            if side == "buy":
                ...
                self.sell_vol -= size; self.current_delta += size
        return {
            "delta": self.current_delta, "buy_vol": self.buy_vol, "sell_vol": self.sell_vol,
...
# ─── Confidence Scorer (unchanged logic) ─────────────────────────────────────
class ConfidenceScorer:
    ...
    def describe_missing(self, sweep: SweepEvent, footprint: Dict, spy_data: Optional[Dict]) -> str:
        """Return description of what would raise confidence."""
        missing = []
        if sweep.sweep_distance < ES_TICK_SIZE * 4:
            missing.append("deeper_sweep(4+ticks)")
        if footprint.get("exhaustion") is None:
            missing.append("delta_exhaustion")
        if spy_data:
            trend = spy_data.get("intraday_return_pct", 0)
            direction_match = (sweep.direction=="bullish_sweep" and trend>0.3) or (sweep.direction=="bearish_sweep" and trend<-0.3)
            if not direction_match:
                missing.append("spy_confirmed_trend")
        return ",".join(missing) if missing else "none"

# ─── Signal Builder (unchanged logic) ────────────────────────────────────────
class SignalBuilder:
    def build_signal(self, sweep: SweepEvent, confidence: int,
        ...
        return AlertSignal(direction=direction, underlying="SPY",
            ...
            instrument=instrument, time_stop=time_stop, exit_rules=exit_rules,
...
# ─── Main Engine — v4.0 with Diagnostics ─────────────────────────────────────
class LiveOrderflowAlertEngine:
    def __init__(self, watch_dir: str, spy_source: str, notify_mode: str,
        ...
        # ─── Diagnostics state ───────────────────────────────────────────────
        self.windows: List[DiagnosticsWindow] = []
        self.current_window = DiagnosticsWindow(window_start=et_now().strftime('%H:%M:%S ET'))
        self.window_interval = 300  # 5 minutes
        self.last_window_time = time.time()
        self.latest_es_price = 0.0
        self.latest_nq_price = 0.0
    ...
    def _maybe_flush_window(self):
        """Write diagnostics window every 5 minutes."""
        now = time.time()
        if now - self.last_window_time >= self.window_interval:
            # Update latest prices
            self.current_window.latest_es_price = self.latest_es_price
            self.current_window.latest_nq_price = self.latest_nq_price
            self.windows.append(self.current_window)
            if len(self.windows) > 100:
                self.windows = self.windows[-50:]
            write_diagnostics_md(self.windows)
            write_diagnostics_json(self.windows)
            self.current_window = DiagnosticsWindow(window_start=et_now().strftime('%H:%M:%S ET'))
            self.last_window_time = now
            _log.info("Diagnostics window written (#%d)", len(self.windows))

    def _log_rejected(self, sweep: SweepEvent, confidence: int, reason: str,
                      footprint: Dict, spy_data: Optional[Dict]) -> None:
        """Log rejected setup to CSV."""
        missing = self.scorer.describe_missing(sweep, footprint, spy_data)
        append_rejected({
            "ts_rejected": et_now().strftime('%Y-%m-%d %H:%M:%S'),
            "symbol": sweep.symbol,
            "setup_type": sweep.direction,
            "confidence": confidence,
            "rejection_reason": reason,
            "missing_confirmations": missing,
            "es_price": self.latest_es_price,
            "nq_price": self.latest_nq_price,
            "spy_state": "cached_bullish" if spy_data else "unavailable",
            "suggestion": f"needs_{missing}",
        })
    ...
    def process_cycle(self) -> List[AlertSignal]:
        ...
        self.current_window.events_processed += len(new_events)
        ...
        for e in new_events:
            ...
            if p:
                if "ES" in sym and "MES" not in sym:
                    ...
                    self.current_window.es_events += 1
                elif "NQ" in sym and "MNQ" not in sym:
                    self.latest_nq_price = p
                    self.current_window.nq_events += 1
        ...
        for sweep in sweeps:
            self.current_window.candidate_sweeps += 1
            confidence = self.scorer.compute(sweep, footprint, spy_data)
            self.current_window.highest_confidence_seen = max(self.current_window.highest_confidence_seen, confidence)
            _log.info("Sweep: %s %s at %.2f (confidence: %d)",
                      sweep.symbol, sweep.direction, sweep.level, confidence)

            if confidence < 20:
                self.current_window.rejected_not_enough_movement += 1
                self._log_rejected(sweep, confidence, "not_enough_movement", footprint, spy_data)
            elif confidence < 75:
                self.current_window.rejected_confidence += 1
                self._log_rejected(sweep, confidence, "confidence_below_threshold", footprint, spy_data)

            signal = self.builder.build_signal(sweep, confidence, spy_data, footprint)
            if signal:
                self.current_window.reclaim_candidates += 1
                signals.append(signal)
                self.signals_generated += 1
            else:
                # Signal was None because confidence < 75; already counted above
                pass
    ...
    def run(self, interval_seconds: float = 5.0):
        ...
        _log.info("=" * 60)
        _log.info("Live Orderflow Alert Engine v4.0")
        ...
        write_status(["Engine v4.0 started", f"SPY source: {self.spy_source}",
        ...
        while self.running:
            ...
            for signal in signals:
                if self.alert_mgr.can_alert(signal):
                    ...
                    self.current_window.rejected_cooldown += 1

            # Update health
            elapsed = time.time() - self.start_time
            events_per_sec = self.total_events / max(elapsed, 1)
            health = HealthStatus(
                capture_ok=True, spy_fresh=True, symbols_ok=True, timestamps_ok=True,
                event_rate_ok=events_per_sec > 0.001, no_duplicates=True,
                last_check=et_now().strftime("%Y-%m-%d %H:%M:%S ET"), reason="Running normally",
                events_per_sec=round(events_per_sec, 3), total_events=self.total_events,
    ...
    def stop(self):
        ...
        elapsed = time.time() - self.start_time
        events_per_sec = self.total_events / max(elapsed, 1)
        write_health(HealthStatus(
            capture_ok=True, last_check=et_now().strftime("%Y-%m-%d %H:%M:%S ET"),
            reason="Stopped gracefully", events_per_sec=round(events_per_sec, 3),
            total_events=self.total_events, signals_generated=self.signals_generated,
        ))
...
# ─── CLI ─────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Live Orderflow Alert Engine v4.0")
    ...
    engine = LiveOrderflowAlertEngine(
        watch_dir=args.watch, spy_source=args.spy_source, notify_mode=args.notify,
        confidence_threshold=args.confidence_threshold, cooldown_minutes=args.cooldown_minutes,
        dry_run=args.dry_run, start_at_end=args.start_at_end,
    )
    try:
